...
When your data has no reference answers, omit `output_field` (or use `output_field=None`).
Use `LLMJudge` or a `CustomMetric` — reference-based metrics (`RougeScore`, `BleuScore`,
`ExactMatch`) will raise a clear error upfront.
